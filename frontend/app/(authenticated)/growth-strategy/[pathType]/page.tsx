...
export default function PathDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 py-8">
      {/* Header */}
      <div>
        <Link href="/growth-strategy" className="text-sm text-gray-500 hover:text-gray-700">
          ← Growth Strategy
        </Link>
      ...
      {path.strategy && (
        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <h2 className="text-sm font-semibold uppercase text-gray-500">Active Strategy</h2>
          <div className="mt-2 flex items-center justify-between">
            <span className="font-medium text-gray-900">{path.strategy.name}</span>
          ...
          <div className="mt-3 grid grid-cols-3 gap-4 text-center text-sm">
            <div>
              ...
              <div className="text-gray-500">Action Items</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-gray-500">Micro Tasks</div>
            </div>
          </div>
        </div>
      )}
      ...
      {scalingPlan && (
        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <h2 className="text-sm font-semibold uppercase text-gray-500">Scaling Vision</h2>
      ...
      {!scalingPlan && Object.keys(content).length > 0 && (
        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <h2 className="text-sm font-semibold uppercase text-gray-500">Path Content</h2>
            ...
            {sortedActions.map((item) => (
              <label
                ...
                className={`flex cursor-pointer items-start gap-3 rounded-md px-3 py-2.5 transition-colors ${
                  item.is_completed ? "bg-gray-50" : "hover:bg-gray-50"
                }`}
              >
                <input
                  type="checkbox"
                ...
                <div className="flex-1">
                  <div className={`text-sm font-medium ${item.is_completed ? "text-gray-400 line-through" : "text-gray-900"}`}>
                    {item.title}
                  </div>
                  {item.description && (
